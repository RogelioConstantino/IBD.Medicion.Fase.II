...
$(".btn[data-target='#edit']").click(function () {
    ...
    $.each(columnHeadings, function (i, columnHeader) {
        ...
        if (i <= campos) {
            ...
            else if ((columnHeader == 'Clave')) {
                formGroup.append('<label for="' + columnHeader + '">' + columnHeader + '</label>');
                formGroup.append('<input type="text" class="form-control text-uppercase" name="' + columnHeader + '" id="' + columnHeader + '" value="' + Value + '"   maxlength="2" />');
            }
            else {
                if (columnHeader != '') {
                    formGroup.append('<label for="' + columnHeader + '">' + columnHeader + '</label>');
